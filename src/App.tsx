...
import { ArrowDownToLine, CalendarDays, Check, CircleHelp, Clock3, FileJson, History, Landmark, Link2, Menu, Pencil, Plus, Printer, ReceiptText, Settings2, ShieldCheck, Sparkles, Target, Trash2, TrendingDown, Upload, WalletCards, X } from 'lucide-react'
...
const scenarioName = (scenario: string) => scenario === 'base' ? 'Без досрочных платежей' : scenario === 'reduceTerm' ? 'Сократить срок' : scenario === 'reducePayment' ? 'Снизить платёж' : scenario === 'combined' ? 'По операциям' : scenario === 'full' ? 'Полное погашение' : scenario === 'custom' ? 'Комбинированная стратегия' : scenario
const sourceName = (source: EarlyRepayment['source']) => source === 'own' ? 'Собственные средства' : source === 'subsidy' ? 'Маткапитал / субсидия' : source === 'insurance' ? 'Страховое возмещение' : 'Прочее'
const ruleTypeName = (type: RepaymentRule['type']) => type === 'monthlyFixed' ? 'Каждый месяц фиксированная сумма' : type === 'annualBonus' ? 'Ежегодная премия' : 'Процент от регулярного платежа'
const graceTypeName = (type: GracePeriod['type']) => type === 'full' ? 'Полная отсрочка' : type === 'interestOnly' ? 'Только проценты' : type === 'reduced' ? 'Уменьшенный платёж' : 'Индивидуальный платёж'
const dayCountBasisLabel = (value: LoanConfig['interest']['dayCountBasis']) => value === 'actualActual' ? 'фактические дни / фактический год' : value === 'actual365' ? 'фактические дни / 365 дней' : `${value} дней`
...
function App() {
  ...
  const download = (kind: 'csv' | 'json' | 'xls', loanId = exportLoanId) => {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([body], { type })); a.download = `credit-${safeName}-${calculation.selected.id}.${ext}`; a.click(); URL.revokeObjectURL(a.href)
  }
  ...
  const createLoanFromSharedCalculation = () => {
    ...
    store.addLoanFromData(sharedCalculation)
    setRows(18)
    setSharedCalculation(null)
    setImportStatus({ kind: 'success', text: 'Создан новый кредит из ссылки' })
    clearSharedHash()
  }
  ...
  const replaceActiveWithSharedCalculation = () => {
    ...
    setRows(18)
    setSharedCalculation(null)
    setImportStatus({ kind: 'success', text: 'Текущий кредит заменён данными из ссылки' })
    clearSharedHash()
  }
  ...
  return <div className="app-shell" data-theme={store.theme} data-ui-font={store.appFontSize} data-schedule-font={store.scheduleFontSize}>
    <aside className={mobileNav ? 'sidebar open' : 'sidebar'}>
      <div className="brand"><div className="brand-mark"><Landmark size={22}/></div><div><b>Кредитный калькулятор</b><span>версия {APP_VERSION}</span></div><button className="icon-btn close-nav" onClick={() => setMobileNav(false)}><X/></button></div>
    ...
    <main>
      <header><button className="icon-btn menu-btn" onClick={() => setMobileNav(true)}><Menu/></button><div className="header-title"><p>Финансовый план · v{APP_VERSION}</p><h1>{section === 'overview' ? 'Ваш кредит' : nav.find(x => x[0] === section)?.[2]}</h1></div><LoanSwitcher loans={store.loans} activeLoanId={store.activeLoanId} switchLoan={store.switchLoan} createLoan={store.createLoan} renameLoan={store.renameLoan}/><FontControls appFontSize={store.appFontSize} scheduleFontSize={store.scheduleFontSize} setAppFontSize={store.setAppFontSize} setScheduleFontSize={store.setScheduleFontSize}/><div className="header-actions"><span className="status-dot"><i/> Данные сохранены</span><button className="ghost print-action" onClick={() => window.print()}><Printer size={16}/> Печать</button><button className="primary add-payment-action" onClick={() => openEarly()}><Plus size={17}/> Досрочный платёж</button></div></header>
      <div className="content">
        ...
        {section === 'export' && <ExportPanel loans={store.loans} exportLoanId={exportLoanId} setExportLoanId={setExportLoanId} download={download} importJson={importJson} copyShareLink={copyShareLink} status={importStatus}/>}
...
function LoanSwitcher({ loans, activeLoanId, switchLoan, createLoan, renameLoan }: { loans: LoanProfile[]; activeLoanId: string; switchLoan: (id: string) => void; createLoan: (name?: string) => void; renameLoan: (id: string, name: string) => void }) {
...
function Changelog() {
  return <section className="panel changelog-panel"><div className="panel-head"><div><h3>Что изменилось</h3><p>Версия приложения: {APP_VERSION}. Здесь можно быстро понять, что появилось в последних обновлениях.</p></div></div><div className="changelog-list">{CHANGELOG.map(entry => <article key={entry.version} className="changelog-entry"><div className="date-tile"><b>{entry.date.slice(0, 2)}</b><span>{entry.date.slice(3, 5)}.{entry.date.slice(6)}</span></div><div><span className="eyebrow">v{entry.version} · {entry.date}</span><h4>{entry.title}</h4><ul>{entry.items.map(item => <li key={item}>{item}</li>)}</ul></div></article>)}</div></section>
}
...
function ExportPanel({ loans, exportLoanId, setExportLoanId, download, importJson, copyShareLink, status }: { loans: LoanProfile[]; exportLoanId: string; setExportLoanId: (id: string) => void; download: (x: 'csv'|'json'|'xls', loanId?: string) => void; importJson: (file: File) => void; copyShareLink: () => void; status: { kind: 'success' | 'error'; text: string } | null }) {
  const inputRef = useRef<HTMLInputElement>(null)
  return <section className="panel export-panel"><div className="panel-head"><div><h3>Импорт/экспорт расчёта</h3><p>Выберите кредит, сохраните резервную копию, передайте ссылку или восстановите расчёт из JSON</p></div></div><div className="export-target"><Field label="Какой кредит выгружать"><select value={exportLoanId} onChange={event => setExportLoanId(event.target.value)}>{loans.map(loan => <option key={loan.id} value={loan.id}>{loan.name}</option>)}</select></Field></div><div className="export-grid"><button onClick={() => download('csv', exportLoanId)}><span className="export-icon green"><ReceiptText/></span><b>CSV</b><small>График для таблиц</small><ArrowDownToLine/></button><button onClick={() => download('xls', exportLoanId)}><span className="export-icon emerald"><Landmark/></span><b>Excel</b><small>Совместимо с .xls</small><ArrowDownToLine/></button><button onClick={() => download('json', exportLoanId)}><span className="export-icon violet"><FileJson/></span><b>Сохранить JSON</b><small>Резервная копия выбранного кредита</small><ArrowDownToLine/></button><button onClick={() => inputRef.current?.click()}><span className="export-icon import"><Upload/></span><b>Загрузить JSON</b><small>Заменить текущий кредит</small><Upload/></button><button onClick={copyShareLink}><span className="export-icon link"><Link2/></span><b>Ссылка на расчёт</b><small>Передать выбранный кредит и платежи</small><Link2/></button><button onClick={() => window.print()}><span className="export-icon amber"><Printer/></span><b>PDF / печать</b><small>Печатная версия текущего экрана</small><ArrowDownToLine/></button></div><input ref={inputRef} className="file-input" type="file" accept="application/json,.json" onChange={event => { const file = event.target.files?.[0]; if (file) importJson(file); event.currentTarget.value = '' }}/>{status && <div className={`import-status ${status.kind}`} role="status" aria-live="polite">{status.kind === 'success' ? <Check/> : <CircleHelp/>}{status.text}</div>}</section>
}

function SharedCalculationModal({ data, createNew, replaceCurrent, decline }: { data: LoanBackupData; createNew: () => void; replaceCurrent: () => void; decline: () => void }) {
  return <div className="modal-backdrop"><div className="modal shared-modal"><div className="modal-head"><div><span className="eyebrow">Ссылка на расчёт</span><h2>Загрузить кредит из ссылки?</h2></div><button className="icon-btn" onClick={decline}><X/></button></div><div className="modal-body"><p className="share-warning">Ссылка содержит параметры кредита, досрочные платежи и льготные периоды. Можно создать новый кредит или заменить текущий открытый кредит.</p><dl className="share-summary">{data.name && <div><dt>Название</dt><dd>{data.name}</dd></div>}<div><dt>Сумма кредита</dt><dd>{money(data.config.principal)}</dd></div><div><dt>Ставка</dt><dd>{data.config.annualRate}%</dd></div><div><dt>Дата выдачи</dt><dd>{shortDate(data.config.issueDate)}</dd></div><div><dt>Первый платёж</dt><dd>{shortDate(data.config.firstPaymentDate)}</dd></div><div><dt>Срок</dt><dd>{fmtMonthsFull(data.config.termMonths)}</dd></div><div><dt>Досрочные платежи</dt><dd>{data.repayments.length}</dd></div><div><dt>Льготные периоды</dt><dd>{data.gracePeriods.length}</dd></div><div><dt>Сценарий</dt><dd>{scenarioName(data.selectedScenario)}</dd></div></dl></div><div className="modal-actions"><button className="ghost" onClick={decline}>Отказаться</button><button className="ghost" onClick={replaceCurrent}>Заменить текущий</button><button className="primary" onClick={createNew}>Создать новый кредит</button></div></div></div>
}
...
function PrintReport({ config, repayments, comparison, selected }: any) {
  ...
  return <article className="print-report">
    ...
    {repayments.length ? <table><thead><tr><th>Дата</th><th>Сумма</th><th>Стратегия</th><th>Комментарий</th></tr></thead><tbody>{repayments.map((r: EarlyRepayment) => <tr key={r.id}><td>{shortDate(r.date)}</td><td>{money(r.amount)}</td><td>{scenarioName(r.strategy)}</td><td>{r.comment || '—'}</td></tr>)}</tbody></table> : <p className="print-muted">Досрочные платежи не добавлены.</p>}
...
function Empty({ icon, title, action }: any) { return <div className="empty"><span>{icon}</span><h3>{title}</h3><p>Добавьте событие, и мы сразу покажем его влияние на кредит.</p><button className="ghost" onClick={action}><Plus/> Добавить</button></div> }
