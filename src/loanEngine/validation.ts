...
export function validateScenario(config: LoanConfig, repayments: EarlyRepayment[], gracePeriods: GracePeriod[]) {
  ...
  repayments.forEach((repayment, index) => {
    if (!finite(repayment.amount) || repayment.amount <= 0) errors.push(`Досрочный платёж №${index + 1}: сумма должна быть больше нуля`)
    ...
    if (isTotalMode && repayment.sameDayOrder === 'earlyFirst') errors.push(`Досрочный платёж №${index + 1}: общая сумма по телу и процентам без комиссий может применяться только после регулярного платежа`)
    if (repayment.amountMode === 'total' && isISODate(repayment.date) && !isRegularDate) errors.push(`Досрочный платёж №${index + 1}: общую сумму по телу и процентам без комиссий можно указать только в дату регулярного платежа`)
    if (isTotalMode && isRegularDate) totalRepaymentsByDate.set(repayment.date, (totalRepaymentsByDate.get(repayment.date) ?? 0) + 1)
  })
