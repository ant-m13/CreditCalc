...
export function parseLoanBackupObject(raw: unknown): LoanBackupData {
  ...
  if (!finite(config.principal) || !finite(config.annualRate) || !finite(config.termMonths, 1) || config.termMonths > MAX_TERM_MONTHS || !finite(config.paymentDay, 1) || config.paymentDay > 31 || !finite(config.closeThreshold) || !finite(config.oneTimeFee) || !finite(config.monthlyFee) || !finite(config.earlyRepaymentFeePercent)) throw new Error('Параметры кредита содержат недопустимые числа')
