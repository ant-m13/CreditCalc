...
const immutableDefaultConfig: LoanConfig = {
  principal: 7200000, annualRate: 12.4, rateChanges: [], rateChangeMode: 'nextPeriod', issueDate: '2026-06-23', firstPaymentDate: '2026-07-15', firstPaymentInterestOnly: true, firstPaymentInterestOnlyMode: 'addToTerm', termMonths: 240,
...
export const createDefaultConfig = (today = new Date(), paymentDay = immutableDefaultConfig.paymentDay): LoanConfig => {
  ...
  const normalizedPaymentDay = Math.min(31, Math.max(1, Math.round(paymentDay)))
