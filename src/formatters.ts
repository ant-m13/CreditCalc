...
// Числа в условиях соответствуют правилам русских окончаний: исключаем 11–14 и учитываем последнюю цифру.
...
export const fmtMonths = (n: number) => { const years = Math.floor(n / MONTHS_PER_YEAR), months = n % MONTHS_PER_YEAR; return [years ? `${years} ${plural(years, 'год', 'года', 'лет')}` : '', months ? `${months} ${plural(months, 'месяц', 'месяца', 'месяцев')}` : ''].filter(Boolean).join(' ') || '0 месяцев' }
