...
function MobileScheduleCard({ row, showFees, showDeferred, money, expandedRows, toggleRow, idPrefix = 'mobile-schedule-row' }: { row: PaymentScheduleItem; showFees: boolean; showDeferred: boolean; money: (value: number, compact?: boolean) => string; expandedRows?: Set<number>; toggleRow?: (number: number) => void; idPrefix?: string }) {
  const auditExpanded = expandedRows?.has(row.number) ?? false
  return <article id={`${idPrefix}-${row.number}`} className={row.event ? 'schedule-card recalc-row' : 'schedule-card'}><div><span>№ {row.number}</span><b>{shortDate(row.date)}</b></div><dl><div><dt>По кредиту</dt><dd>{money(row.principalPaid ?? row.principal)}</dd></div><div><dt>Проценты</dt><dd>{money(row.interestPaid ?? row.interest)}</dd></div>{showFees && <div><dt>Комиссия</dt><dd>{money(rowFee(row))}</dd></div>}<div><dt>Итого</dt><dd>{money(rowTotal(row))}</dd></div><div><dt>Остаток</dt><dd>{money(row.closingBalance)}</dd></div>{showDeferred && <><div><dt>Отложенные проценты</dt><dd>{money(rowDeferredInterest(row))}</dd></div><div><dt>Общая задолженность</dt><dd>{money(rowTotalDebt(row))}</dd></div></>}</dl>{row.audit && expandedRows && toggleRow && <button className="ghost compact" onClick={() => toggleRow(row.number)} aria-expanded={auditExpanded} aria-controls={`${idPrefix}-audit-${row.number}`}>{auditExpanded ? 'Скрыть формулу' : 'Показать формулу'}</button>}{row.audit && auditExpanded && <div id={`${idPrefix}-audit-${row.number}`} className="mobile-audit"><AuditCard row={row} money={money}/></div>}</article>
}
...
function ScheduleTable({ rows, expandedRows, toggleRow, showFees, showDeferred, money }: { rows: PaymentScheduleItem[]; expandedRows: Set<number>; toggleRow: (number: number) => void; showFees: boolean; showDeferred: boolean; money: (value: number, compact?: boolean) => string }) {
  ...
  return <table className="bank-schedule"><caption className="sr-only">График платежей: даты, погашение основного долга, проценты, комиссии, итоговое списание и остаток задолженности после каждой строки.</caption><thead><tr><th rowSpan={2}>№ п/п</th><th rowSpan={2}>Дата</th><th colSpan={showFees ? 4 : 3}>Сумма платежа</th><th rowSpan={2}>Остаток задолженности</th>{showDeferred && <><th rowSpan={2}>Отложенные проценты</th><th rowSpan={2}>Общая задолженность</th></>}</tr><tr><th>По кредиту</th><th>По процентам</th>{showFees && <th>Комиссия</th>}<th>Итого</th></tr></thead><tbody>{rows.flatMap(row => [<tr id={`schedule-row-${row.number}`} key={`${row.number}-${row.date}`} className={row.event ? 'recalc-row' : ''}><td data-label="№"><span className="schedule-cell-value">{row.audit ? <button className="audit-toggle" onClick={() => toggleRow(row.number)} aria-label={`${expandedRows.has(row.number) ? 'Скрыть' : 'Показать'} формулу строки ${row.number}`} aria-expanded={expandedRows.has(row.number)} aria-controls={`audit-row-${row.number}`}>{expandedRows.has(row.number) ? '−' : '+'}</button> : <span className="audit-spacer" aria-hidden="true"/>}{row.number}</span></td><td data-label="Дата">{shortDate(row.date)}</td><td data-label="По кредиту">{money(row.principalPaid ?? row.principal)}</td><td data-label="По процентам">{money(row.interestPaid ?? row.interest)}</td>{showFees && <td data-label="Комиссия">{money(rowFee(row))}</td>}<td data-label="Итого">{money(rowTotal(row))}</td><td data-label="Остаток"><b>{money(row.closingBalance)}</b></td>{showDeferred && <><td data-label="Отложенные проценты">{money(rowDeferredInterest(row))}</td><td data-label="Общая задолженность"><b>{money(rowTotalDebt(row))}</b></td></>}</tr>, ...(row.audit && expandedRows.has(row.number) ? [<AuditDetails row={row} colSpan={colSpan} money={money} key={`audit-${row.number}-${row.date}`}/>] : [])])}</tbody></table>
}

const collapsedColumnCount = (showFees: boolean, showDeferred: boolean) => 7 + (showFees ? 1 : 0) + (showDeferred ? 2 : 0)

export function Schedule({ schedule, baseSchedule, config, gracePeriods, currency, displayDecimals, rows, setRows }: ScheduleProps) {
  ...
  return <section className="panel table-panel">
    ...
    <div className={mobileTableMode ? 'table-wrap force-mobile-table mobile-schedule-table-wrap' : 'table-wrap'}>
      {monthsCollapsed ? <table className="bank-schedule"><caption className="sr-only">Свёрнутый график платежей по месяцам: количество строк, суммы погашения, проценты, комиссии, итог и задолженность на конец месяца.</caption><thead><tr><th>Месяц</th><th>Строк</th><th>По кредиту</th><th>По процентам</th>{showFees && <th>Комиссия</th>}<th>Итого</th><th>Остаток задолженности</th>{showDeferred && <><th>Отложенные проценты</th><th>Общая задолженность</th></>}<th>Действие</th></tr></thead><tbody>{groupedRows.map(group => {
        const groupRow = <tr key={group.key} className="month-row"><td data-label="Месяц">{group.title}</td><td data-label="Строк">{group.rows.length}</td><td data-label="По кредиту">{money(group.totals.principal)}</td><td data-label="По процентам">{money(group.totals.interest)}</td>{showFees && <td data-label="Комиссия">{money(group.totals.fee)}</td>}<td data-label="Итого">{money(group.totals.total)}</td><td data-label="Остаток">{money(group.totals.closing)}</td>{showDeferred && <><td data-label="Отложенные проценты">{money(group.totals.deferred)}</td><td data-label="Общая задолженность">{money(group.totals.debt)}</td></>}<td data-label="Действие"><button className="ghost compact" onClick={() => toggleMonth(group.key)}>{openMonths.has(group.key) ? 'Свернуть' : 'Открыть'}</button></td></tr>
        if (!openMonths.has(group.key)) return [groupRow]
        const colSpan = collapsedColumnCount(showFees, showDeferred)
        return [groupRow, ...group.rows.flatMap(item => [<tr id={`schedule-row-${item.number}`} key={`${item.number}-${item.date}`} className={item.event ? 'recalc-row detail-row' : 'detail-row'}><td data-label="Дата">{item.audit ? <button className="audit-toggle" onClick={() => toggleRow(item.number)} aria-label={`${expandedRows.has(item.number) ? 'Скрыть' : 'Показать'} формулу строки ${item.number}`} aria-expanded={expandedRows.has(item.number)} aria-controls={`audit-row-${item.number}`}>{expandedRows.has(item.number) ? '−' : '+'}</button> : <span className="audit-spacer" aria-hidden="true"/>}{shortDate(item.date)}</td><td data-label="№">№ {item.number}</td><td data-label="По кредиту">{money(item.principalPaid ?? item.principal)}</td><td data-label="По процентам">{money(item.interestPaid ?? item.interest)}</td>{showFees && <td data-label="Комиссия">{money(rowFee(item))}</td>}<td data-label="Итого">{money(rowTotal(item))}</td><td data-label="Остаток">{money(item.closingBalance)}</td>{showDeferred && <><td data-label="Отложенные проценты">{money(rowDeferredInterest(item))}</td><td data-label="Общая задолженность">{money(rowTotalDebt(item))}</td></>}<td data-label="Действие"></td></tr>, ...(item.audit && expandedRows.has(item.number) ? [<AuditDetails row={item} colSpan={colSpan} money={money} key={`audit-${item.number}-${item.date}`}/>] : [])])]
    ...
    {!mobileTableMode && !monthsCollapsed && <div className="mobile-schedule-cards">{visibleRows.map(row => <MobileScheduleCard row={row} showFees={showFees} showDeferred={showDeferred} money={money} expandedRows={expandedRows} toggleRow={toggleRow} key={`card-${row.number}-${row.date}`}/>)}</div>}
    {!mobileTableMode && monthsCollapsed && <div className="mobile-schedule-cards mobile-month-cards">{groupedRows.map(group => <article className="schedule-card mobile-month-card" key={`mobile-month-${group.key}`}><div><span>{group.rows.length} {plural(group.rows.length, 'строка', 'строки', 'строк')}</span><b>{group.title}</b></div><dl><div><dt>По кредиту</dt><dd>{money(group.totals.principal)}</dd></div><div><dt>Проценты</dt><dd>{money(group.totals.interest)}</dd></div>{showFees && <div><dt>Комиссия</dt><dd>{money(group.totals.fee)}</dd></div>}<div><dt>Итого</dt><dd>{money(group.totals.total)}</dd></div><div><dt>Остаток</dt><dd>{money(group.totals.closing)}</dd></div>{showDeferred && <><div><dt>Отложенные проценты</dt><dd>{money(group.totals.deferred)}</dd></div><div><dt>Общая задолженность</dt><dd>{money(group.totals.debt)}</dd></div></>}</dl><button className="ghost compact" onClick={() => toggleMonth(group.key)} aria-expanded={openMonths.has(group.key)}>{openMonths.has(group.key) ? 'Скрыть платежи месяца' : 'Показать платежи месяца'}</button>{openMonths.has(group.key) && <div className="mobile-month-rows">{group.rows.map(item => <MobileScheduleCard row={item} showFees={showFees} showDeferred={showDeferred} money={money} expandedRows={expandedRows} toggleRow={toggleRow} idPrefix="mobile-month-schedule-row" key={`mobile-month-row-${item.number}-${item.date}`}/>)}</div>}</article>)}</div>}
    {!monthsCollapsed && <><table className="schedule-totals bank-schedule desktop-schedule-totals"><tfoot><tr><td colSpan={2}>Итого за весь срок</td><td>{money(totals.principal)}</td><td>{money(totals.interest)}</td>{showFees && <td>{money(totals.fee)}</td>}<td>{money(totals.total)}</td><td>{money(closingRow?.closingBalance ?? 0)}</td>{showDeferred && <><td>{money(closingRow?.deferredInterestClosing ?? 0)}</td><td>{money(closingRow ? rowTotalDebt(closingRow) : 0)}</td></>}</tr></tfoot></table><section className="mobile-schedule-total" aria-label="Итого за весь срок"><b>Итого за весь срок</b><dl><div><dt>По кредиту</dt><dd>{money(totals.principal)}</dd></div><div><dt>Проценты</dt><dd>{money(totals.interest)}</dd></div>{showFees && <div><dt>Комиссия</dt><dd>{money(totals.fee)}</dd></div>}<div><dt>Всего выплачено</dt><dd>{money(totals.total)}</dd></div><div><dt>Остаток</dt><dd>{money(closingRow?.closingBalance ?? 0)}</dd></div>{showDeferred && <><div><dt>Отложенные проценты</dt><dd>{money(closingRow?.deferredInterestClosing ?? 0)}</dd></div><div><dt>Общая задолженность</dt><dd>{money(closingRow ? rowTotalDebt(closingRow) : 0)}</dd></div></>}</dl></section></>}
