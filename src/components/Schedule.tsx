...
function ScheduleTable({ rows, expandedRows, toggleRow, showFees }: { rows: PaymentScheduleItem[]; expandedRows: Set<number>; toggleRow: (number: number) => void; showFees: boolean }) {
  return <table className="bank-schedule"><thead><tr><th rowSpan={2}>№ п/п</th><th rowSpan={2}>Дата</th><th colSpan={showFees ? 4 : 3}>Сумма платежа</th><th rowSpan={2}>Остаток задолженности</th></tr><tr><th>По кредиту</th><th>По процентам</th>{showFees && <th>Комиссия</th>}<th>Итого</th></tr></thead><tbody>{rows.flatMap(row => [<tr id={`schedule-row-${row.number}`} key={`${row.number}-${row.date}`} className={row.event ? 'recalc-row' : ''}><td><button className="audit-toggle" onClick={() => toggleRow(row.number)} aria-label={`Показать формулу строки ${row.number}`}>{expandedRows.has(row.number) ? '−' : '+'}</button>{row.number}</td><td>{shortDate(row.date)}</td><td>{money(row.principalPaid ?? row.principal)}</td><td>{money(row.interestPaid ?? row.interest)}</td>{showFees && <td>{money(rowFee(row))}</td>}<td>{money(rowTotal(row))}</td><td><b>{money(row.closingBalance)}</b></td></tr>, ...(expandedRows.has(row.number) ? [<AuditDetails row={row} showFees={showFees} key={`audit-${row.number}-${row.date}`}/>] : [])])}</tbody></table>
}
...
export function Schedule({ schedule, baseSchedule, repayments, rows, setRows, more }: ScheduleProps) {
  ...
  return <section className="panel table-panel">
    ...
    <div className={mobileTableMode ? 'table-wrap force-mobile-table' : 'table-wrap'}>
      {monthsCollapsed ? <table className="bank-schedule"><thead><tr><th>Месяц</th><th>Строк</th><th>По кредиту</th><th>По процентам</th>{showFees && <th>Комиссия</th>}<th>Итого</th><th>Действие</th></tr></thead><tbody>{groupedRows.map(group => <tr key={group.key} className="month-row"><td>{group.title}</td><td>{group.rows.length}</td><td>{money(group.totals.principal)}</td><td>{money(group.totals.interest)}</td>{showFees && <td>{money(group.totals.fee)}</td>}<td>{money(group.totals.total)}</td><td><button className="ghost compact" onClick={() => toggleMonth(group.key)}>{openMonths.has(group.key) ? 'Свернуть' : 'Открыть'}</button></td></tr>).flatMap((row, index) => openMonths.has(groupedRows[index].key) ? [row, ...groupedRows[index].rows.map(item => <tr id={`schedule-row-${item.number}`} key={`${item.number}-${item.date}`} className={item.event ? 'recalc-row detail-row' : 'detail-row'}><td><button className="audit-toggle" onClick={() => toggleRow(item.number)}>{expandedRows.has(item.number) ? '−' : '+'}</button>{shortDate(item.date)}</td><td>№ {item.number}</td><td>{money(item.principalPaid ?? item.principal)}</td><td>{money(item.interestPaid ?? item.interest)}</td>{showFees && <td>{money(rowFee(item))}</td>}<td>{money(rowTotal(item))}</td><td>{money(item.closingBalance)}</td></tr>)] : [row])}</tbody></table> : <ScheduleTable rows={visibleRows} expandedRows={expandedRows} toggleRow={toggleRow} showFees={showFees}/>}
    </div>
    {!mobileTableMode && !monthsCollapsed && <div className="mobile-schedule-cards">{visibleRows.map(row => <article id={`mobile-schedule-row-${row.number}`} key={`card-${row.number}-${row.date}`} className={row.event ? 'schedule-card recalc-row' : 'schedule-card'}><div><span>№ {row.number}</span><b>{shortDate(row.date)}</b></div><dl><div><dt>По кредиту</dt><dd>{money(row.principalPaid ?? row.principal)}</dd></div><div><dt>Проценты</dt><dd>{money(row.interestPaid ?? row.interest)}</dd></div>{showFees && <div><dt>Комиссия</dt><dd>{money(rowFee(row))}</dd></div>}<div><dt>Итого</dt><dd>{money(rowTotal(row))}</dd></div><div><dt>Остаток</dt><dd>{money(row.closingBalance)}</dd></div></dl>{row.audit && <button className="ghost compact" onClick={() => toggleRow(row.number)}>{expandedRows.has(row.number) ? 'Скрыть формулу' : 'Показать формулу'}</button>}{expandedRows.has(row.number) && <div className="mobile-audit"><AuditCard row={row}/></div>}</article>)}</div>}
    {!monthsCollapsed && <table className="schedule-totals bank-schedule"><tfoot><tr><td colSpan={2}>Итого за весь срок</td><td>{money(totals.principal)}</td><td>{money(totals.interest)}</td>{showFees && <td>{money(totals.fee)}</td>}<td>{money(totals.total)}</td><td>{money(schedule.at(-1)?.closingBalance ?? 0)}</td></tr></tfoot></table>}
