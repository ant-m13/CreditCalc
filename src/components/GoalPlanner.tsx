...
export function GoalPlanner({ loanId, sourceRevision, config, repayments, repaymentRules, gracePeriods, selectedScenario, displayDecimals, disabled = false, applyGoalPlan }: GoalPlannerProps) {
  ...
  return <div className="goal-planner-layout">
    <section className="panel form-panel goal-form-panel">
      ...
      {(loading || previewLoading) && <div className="alert goal-progress" role="status" aria-live="polite">{previewLoading ? 'Строим выбранный график…' : 'Подбираем минимальные суммы в фоновом режиме…'}</div>}
      ...
      {result && showResults && <section className="goal-results goal-results-inline" id="goal-planner-results" aria-live="polite">
        ...
        {showComparison && <div className="table-wrap force-mobile-table goal-comparison" id="goal-planner-comparison" role="region" aria-label="Сравнение вариантов" tabIndex={0}><table><caption className="sr-only">Сравнение рассчитанных вариантов достижения цели.</caption><thead><tr><th>Вариант</th><th>Закрытие</th><th>Проценты</th><th>Комиссии</th><th>Доп. вложения</th><th>Всего банку</th></tr></thead><tbody>{result.variants.filter(item => item.status === 'achieved').map(variant => <tr key={`compare-${variant.kind}`}><td data-label="Вариант">{variant.title}</td><td data-label="Закрытие">{shortDate(variant.summary!.closingDate)}</td><td data-label="Проценты">{money(variant.summary!.total.interest)}</td><td data-label="Комиссии">{money(variant.summary!.total.fees)}</td><td data-label="Доп. вложения">{money(variant.summary!.plannerContribution.additionalInvestment)}</td><td data-label="Всего банку">{money(variant.summary!.total.bankTransfer)}</td></tr>)}</tbody></table></div>}
