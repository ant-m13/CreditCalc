...
export function Overview({ config, repayments, gracePeriods, comparison, selected, chartData, onSelect, onOpen }: { config: LoanConfig; repayments: EarlyRepayment[]; gracePeriods: GracePeriod[]; comparison: ComparisonResult; selected: ScenarioResult; chartData: { date: string; base: number; balance: number | null }[]; onSelect: (id: string) => void; onOpen: () => void }) {
  ...
  return <>
    ...
    <div className="scenario-grid">{comparison.scenarios.slice(1).map((s, i) => <button key={s.id} className={selected.id === s.id ? 'scenario selected' : 'scenario'} onClick={() => onSelect(s.id)}><span className={`scenario-icon c${i}`} >{i === 0 ? <Target/> : i === 1 ? <WalletCards/> : <Sparkles/>}</span><span className="scenario-title">{s.name}{s.id === comparison.bestSavings.id && <em>Макс. экономия процентов</em>}</span><b>{money(s.monthlyPayment)} <small>{paymentPeriodLabel}</small></b><span className="scenario-stats"><i>Экономия <strong>{money(s.interestSavings, true)}</strong></i><i>Срок <strong>−{fmtMonthsFull(s.monthsSaved)}</strong></i></span><span className="radio">{selected.id === s.id && <Check size={14}/>}</span></button>)}</div>
