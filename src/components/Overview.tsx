...
export function Overview({ config, displayDecimals, repayments, gracePeriods, comparison, selected, chartData, onSelect, onOpen }: { config: LoanConfig; displayDecimals: 0 | 2; repayments: EarlyRepayment[]; gracePeriods: GracePeriod[]; comparison: ComparisonResult; selected: ScenarioResult; chartData: { date: string; base: number; balance: number | null }[]; onSelect: (id: string) => void; onOpen: () => void }) {
  ...
  return <>
    ...
    <section className="current-debt-grid">
      {loanNotIssued ? <div className="current-debt main"><span>Кредит ещё не выдан</span><b>{money(config.principal)}</b><small>Выдача запланирована на {shortDate(config.issueDate)}</small></div> : <div className="current-debt main"><span>Остаток долга на {shortDate(debt.date)}</span><b>{money(debt.total)}</b><small>Тело кредита + начисленные проценты после {shortDate(debt.fromDate)}</small></div>}
