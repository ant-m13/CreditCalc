...
export function Overview({ config, displayDecimals, repayments, gracePeriods, comparison, selected, chartData, onSelect, onOpen }: { config: LoanConfig; displayDecimals: 0 | 2; repayments: EarlyRepayment[]; gracePeriods: GracePeriod[]; comparison: ComparisonResult; selected: ScenarioResult; chartData: { date: string; base: number; balance: number | null }[]; onSelect: (id: string) => void; onOpen: () => void }) {
  ...
  const interestPaidPercent = loanNotIssued ? 0 : Math.min(100, interestPaidToDate / Math.max(1, selected.totalInterest) * 100)
  ...
  const milestones = [
    { title: 'Остаток ниже 75%', done: !loanNotIssued && debt.principal <= config.principal * .75 },
    { title: 'Половина кредита погашена', done: !loanNotIssued && debt.principal <= config.principal * .5 },
    { title: 'Последний миллион', done: !loanNotIssued && debt.principal <= 1_000_000 },
    { title: 'Последний год', done: !loanNotIssued && remainingMonths <= 12 },
  ...
  return <>
    ...
    {nextPayment && <section className="panel next-payment-panel"><div className="panel-head"><div><h3>Ближайший платёж</h3><p>{shortDate(nextPayment.date)} · строка №{nextPayment.number}</p></div><b>{money(rowCashFlow(nextPayment))}</b></div><div className="next-payment-grid"><div><span>По кредиту</span><b>{money(nextPayment.principalPaid ?? nextPayment.principal)}</b><small>Погашение тела</small></div><div><span>По процентам</span><b>{money(nextPayment.interestPaid ?? nextPayment.interest)}</b><small>Начисленные проценты</small></div>{Math.abs(nextPaymentFee) > 0.004 && <div><span>Комиссия</span><b>{money(nextPaymentFee)}</b><small>Дополнительный платёж</small></div>}{Math.abs(nextPaymentDeferredInterest) > 0.004 && <div><span>Отложенные проценты</span><b>{money(nextPaymentDeferredInterest)}</b><small>Остаток после платежа</small></div>}<div><span>Остаток после</span><b>{money(nextPayment.closingBalance + nextPaymentDeferredInterest)}</b><small>Общая задолженность</small></div></div></section>}
