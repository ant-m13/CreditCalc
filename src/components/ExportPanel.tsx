...
export function ExportPanel({ loans, exportLoanId, setExportLoanId, download, createFromJson, replaceFromJson, copyShareLink, status }: { loans: LoanProfile[]; exportLoanId: string; setExportLoanId: (id: string) => void; download: (x: 'csv'|'json'|'xls', loanId?: string) => void; createFromJson: (data: LoanBackupData, fileName: string) => boolean; replaceFromJson: (data: LoanBackupData, fileName: string) => boolean; copyShareLink: () => void; status: { kind: 'success' | 'error'; text: string } | null }) {
