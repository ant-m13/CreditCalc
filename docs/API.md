...
| `currency` | `RUB`, `USD`, `EUR` или `CNY`; отсутствующие и неподдерживаемые legacy-значения при импорте нормализуются в `RUB` с предупреждением |
...
- Если поле `currency` отсутствует или legacy JSON/share-link содержит неподдерживаемое значение, приложение нормализует валюту в `RUB` и возвращает предупреждение импорта; суммы при этом не конвертируются. Новые интеграции должны создавать только `RUB`, `USD`, `EUR` или `CNY`.
