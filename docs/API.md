...
JSON-файл создаётся кнопкой экспорта в приложении. Текущий формат основан на снимке `SharedCalculationV1` и дополнительно содержит `exportedAt`.
...
Share-link хранит тот же снимок расчёта, что и JSON, но без поля `exportedAt`.
...
- Если поле `currency` отсутствует, используется `RUB`; если оно присутствует, оно должно быть одним из `RUB`, `USD`, `EUR`, `CNY`.
