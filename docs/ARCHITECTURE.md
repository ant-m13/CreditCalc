...
Скрытие уведомления карантина меняет только UI-флаг: отчёт и recovery-данные остаются до отдельного подтверждённого удаления, а скрытую панель можно открыть снова. Quarantine download называется ограниченной recovery-копией, содержит `rawIsComplete: false`, применённые лимиты и предупреждение о возможном усечении; он не обозначается как полный raw backup.
...
Сам расчёт строится через `LoanCalculationRunner`. В production он использует Web Worker, отправляет immutable snapshot и revision id, а при новой ревизии завершает предыдущий worker-экземпляр до постановки новой задачи. Поэтому latest-wins относится не только к ответам: устаревший тяжёлый расчёт не задерживает актуальный в очереди Worker. Если Worker недоступен или среда тестовая, используется синхронный fallback с тем же чистым модулем `buildLoanCalculation`.
