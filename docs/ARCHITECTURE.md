...
expandRepaymentRules
        ↓
Web Worker: buildLoanCalculation / compareScenarios
        ↓
...
- `src/hooks/useLoanCalculation.ts` — подготовка единого снимка расчёта, запуск Web Worker с revision-id и данные обзора.
...
Сам расчёт строится в Web Worker. Хук расчёта отправляет immutable snapshot и revision id, а ответы со старой ревизией игнорируются. Если Worker недоступен, используется синхронный fallback с тем же чистым модулем `buildLoanCalculation`.
