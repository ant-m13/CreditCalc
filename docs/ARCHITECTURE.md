...
Перед импортом JSON выполняется проверка структуры, дат, чисел, валюты, типов платежей, истории ставки, льготных периодов и регулярных платежей. Поля, которых не было в старых версиях формата, нормализуются через дефолты текущей модели, если их отсутствие не делает расчёт неоднозначным.
