...
Текущая версия persisted-схемы — `6`. При загрузке старого состояния приложение нормализует параметры кредита, разовые платежи, регулярные платежи и льготные периоды. Записи с невозможными датами или повреждёнными обязательными полями отбрасываются, чтобы старый `localStorage` не мог постоянно ломать интерфейс.
