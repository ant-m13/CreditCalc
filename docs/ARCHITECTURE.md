...
Проект проверяется через TypeScript, ESLint, Vitest и production-сборку. ESLint включает правила TypeScript и React Hooks; тесты покрывают расчётное ядро, импорт/экспорт, шаринг ссылкой, миграции store, runner расчёта, валидацию экспорта и базовые UI-сценарии.
